...
カード上のボタンで商品を `候補` または `見送り` に手動仕訳できます。`見送り` の場合はカード上で理由を入力できます。

仕訳記録は `localStorage` の `isa.itemReviews` に保存します。見送り理由は画面上の `仕訳記録` から確認でき、同じ理由の件数もまとめて表示します。スプレッドシートへの書き込みは行いません。
