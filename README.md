...
カード上のボタンで商品を `候補` または `見送り` に手動仕訳できます。`候補に保存` を押すと判定エリア内の `手動仕訳 > 候補` タブに移動します。`見送り` の場合はカード上で理由を入力し、保存ボタンまたは Enter で確定すると `手動仕訳 > 見送り` タブに移動します。

手動仕訳は `localStorage` の `isa.itemReviews` に保存します。スプレッドシートへの書き込みは行いません。
